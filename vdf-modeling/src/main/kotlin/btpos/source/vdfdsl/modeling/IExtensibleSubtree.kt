	...
	object Codegen {
		...
		fun _registerCodegenFieldMapping(propOwner: Any?, prop: KProperty<*>, serializationKey: String, conditional: String?) {
			...
			getOrCreateStructDecoder(receiverType).let {
				it.fieldDecoders.merge(VDFPrimitive(serializationKey), StructFieldDecoderPropExt_Keyed(prop.name, VDFPrimitive(serializationKey), valueType.jvmErasure, operator)) { first, second ->
					first.orElse(second)
				}
			}
		}
		...
		@JvmRecord
		data class DeferredValueDecoder<out T : KtStatement>(val getter: () -> ValueDecoder<T>) : ValueDecoder<T> {
		 ...
		 */
		inline fun <reified T : IExtensibleSubtree> _registerStructFactory(noinline dummyConstructor: (() -> T)? = null, customFieldDecoders: () -> Map<String, CodegenProvider<Decoder<KtExpression>>> = { emptyMap() }, factoryMethod: () -> StructFactoryMethod) {
			if (!IS_DOING_CODEGEN)
				return;
		...
		@PublishedApi internal fun <T : IExtensibleSubtree> _registerStructFactory(cls: KClass<T>, dummyConstructor: (() -> T)?, customFieldDecoders: Map<String, CodegenProvider<Decoder<KtExpression>>>, factoryMethod: StructFactoryMethod) {
			if (!IS_DOING_CODEGEN)
				return;
		...
		data class StructFieldDecoderPropExt_Keyed(val propName: String, val key: VDFPrimitive, val valueType: KClass<*>, val operator: String) : Decoder<KtStatement> {
			private val valueDecoder by lazy {
				Decoders.forType(valueType)
			}
			
			override fun decode(keyvalue: VDFKeyValue): List<KtAssignment> {
				if (keyvalue.key == key) {
					return valueDecoder.decode(keyvalue).map { KtAssignment(KtName(propName), it, operator) }
				}
				return emptyList()
			}
		}
		 ...
		 */
		data class StructFieldDecoderPropExt_SelfNamed(val propName: KtName, val valueType: KClass<*>, val operator: String) : Decoder<KtStatement> {
			private val valueDecoder by lazy {
				Decoders.forType(valueType)
			}
			
			override fun decode(keyvalue: VDFKeyValue): List<KtAssignment> {
				return valueDecoder.decode(keyvalue).map { KtAssignment(propName, it, operator) }
			}
