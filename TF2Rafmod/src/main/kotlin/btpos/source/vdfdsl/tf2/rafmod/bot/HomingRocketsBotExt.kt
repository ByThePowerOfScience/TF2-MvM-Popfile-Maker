...
open class HomingRocketsBotExt(subtree: IExtensibleSubtree_VDFRepresentable = ExtensibleSubtreeImpl()) : RafmodBotExtension(subtree) {
	 ...
	 */
	open val ignoreDisguisedSpies by addField<Boolean>("IgnoreDisguisedSpies", conditional = SIGSEGV)
	
	/**
	 * If true, homing rockets ignore spies that are currently cloaked.
	 */
	open val ignoreCloakedSpies by addField<Boolean>("IgnoreStealthedSpies", conditional = SIGSEGV)
	 ...
	 * ```
	 */
	open val rocketSpeedMult by addField<Number>("RocketSpeed", conditional = SIGSEGV)
	 ...
	 * ```
	 */
	open val turnPower by addField<Number>("TurnPower", conditional = SIGSEGV)
	 ...
	 * ```
	 */
	open val maxErrorBeforeDisengaging by addField<Number>("MaxAimError", conditional = SIGSEGV)
}