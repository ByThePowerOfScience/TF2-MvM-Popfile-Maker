...
abstract class RafmodBotBehavior {
	 ...
	 */
	open var WaveSchedule.canBotShieldsDamagePlayers: Boolean? by addField("MedigunShieldDamage", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
	
	/**
	 * If true, robots will not push players off of their heads.
	 */
	open var WaveSchedule.canPlayersStandOnRobotHeads: Boolean? by addField("StandableHeads", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
	 ...
	 */
	open var WaveSchedule.disableRomevision: Boolean? by addField("NoRomevisionCosmetics", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
	 ...
	 */
	open var WaveSchedule.canBotsRandomCrit: Boolean? by addField("BotsRandomCrit", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
	 ...
	 */
	open var WaveSchedule.canSniperBotsHeadshot: Boolean? by addField("SniperAllowHeadshots", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
	
	/**
	 * If true, robot Pyros can deflect arrows, grenades, and stickybombs.
	 */
	open var WaveSchedule.enableImprovedAirblast: Boolean? by addField("ImprovedAirblast", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
	
	/**
	 * If true, Spy bots will not target unowned buildings.
	 */
	open var WaveSchedule.noSapUnownedBuildings: Boolean? by addField("NoSapUnownedBuildings", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
	 ...
	 */
	open var WaveSchedule.canSandmanStunPlayers: Boolean? by addField("SandmanStun", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
	
	/**
	 * If true, snipers will not display lasers when aiming.
	 */
	open var WaveSchedule.isSniperLaserVisible: Boolean? by addField("SniperHideLasers", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
	
	/**
	 * If true, robots are able to bleed.
	 */
	open var WaveSchedule.canRobotsBleed: Boolean? by addField("ForceRobotBleed", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
}
