...
abstract class RafmodModelsAndAnimations {
	 ...
	 */
	var WaveSchedule.robotsUsePlayerAnimation: Boolean? by addField("PlayerRobotsUsePlayerAnimation", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
	
	/**
	 * If true, fixes the lack of player animations after firing the [setCustomModel] input on a player.
	 */
	var WaveSchedule.fixSetCustomModelInput: Boolean? by addField("FixSetCustomModelInput", conditional = SIGSEGV, serializer = BinaryIntCodec::write)
	 ...
	 * ```
	 */
	var WaveSchedule.botsAreHumans: Int? by addField("BotsAreHumans", conditional = SIGSEGV)
	 ...
	 * ```
	 */
	var WaveSchedule.bodyPartScaleSpeed: Number? by addField("BodyPartScaleSpeed", conditional = SIGSEGV)
}