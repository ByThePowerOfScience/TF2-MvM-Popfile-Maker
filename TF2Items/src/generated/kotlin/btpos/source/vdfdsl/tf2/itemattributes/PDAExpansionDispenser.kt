...
interface PDAExpansionDispenserAttributes : IBlockScoped, WearableAttributes {
	 ...
	 * For outgoing damage, see [damage].
	 */
	override val resistance: ResistanceAttributes get() = PDAExpansionDispenserAttributes.resistance
	
	/**
	 * Attributes related to the scoreboard, killfeed, HuD, item descriptions, and interactions with the wider game-state. (including capture rate)
	 */
	override val meta: MetaAttributes get() = PDAExpansionDispenserAttributes.meta
	
	/**
	 * Attributes related to max ammo, clip-size, and resupply.
	 */
	override val ammo: AmmoAttributes get() = PDAExpansionDispenserAttributes.ammo
	
	override val buffItems: BuffItemsAttributes get() = PDAExpansionDispenserAttributes.buffItems
	
	/**
	 * Attributes related to moving, constructing, and interacting with the Engineer's buildings.
	 */
	override val buildings: BuildingsAttributes get() = PDAExpansionDispenserAttributes.buildings
	
	override val cloak: CloakAttributes get() = PDAExpansionDispenserAttributes.cloak
	 ...
	 * For damage _taken_, see [resistance].
	 */
	override val damage: DamageAttributes get() = PDAExpansionDispenserAttributes.damage
	 ...
	 * Some of these attributes are hardcoded to only work on the Demoman. These are noted in their documentation.
	 */
	override val demoCharge: DemoChargeAttributes get() = PDAExpansionDispenserAttributes.demoCharge
	
	/**
	 * Attributes related to disguising.
	 */
	override val disguise: DisguiseAttributes get() = PDAExpansionDispenserAttributes.disguise
	
	/**
	 * Attributes governing rate-of-fire.
	 */
	override val firing: FiringAttributes get() = PDAExpansionDispenserAttributes.firing
	 ...
	 * For "revenge crits", like the Frontier Justice, Manmelter, and Diamondback, see [revengeCrits].
	 */
	override val heads: HeadsAttributes get() = PDAExpansionDispenserAttributes.heads
	
	/**
	 * Attributes related to the player's HP stat and healing players.
	 */
	override val healthAndHealing: HealthAndHealingAttributes get() = PDAExpansionDispenserAttributes.healthAndHealing
	
	override val hud: HudAttributes get() = PDAExpansionDispenserAttributes.hud
	
	/**
	 * Attributes governing how much you are pushed when hit by different push sources.
	 */
	override val knockbackReceived: KnockbackReceivedAttributes get() = PDAExpansionDispenserAttributes.knockbackReceived
	
	/**
	 * Attributes related to rage and items that recharge on a meter/timer.
	 */
	override val meter: MeterAttributes get() = PDAExpansionDispenserAttributes.meter
	
	/**
	 * Attributes governing the player's move-speed, jump height, swimming, air-strafing capabilities, and all things mobility-related.
	 */
	override val movement: MovementAttributes get() = PDAExpansionDispenserAttributes.movement
	 ...
	 * @see onKill
	 */
	override val onHit: OnHitAttributes get() = PDAExpansionDispenserAttributes.onHit
	 ...
	 * @see onHit
	 */
	override val onKill: OnKillAttributes get() = PDAExpansionDispenserAttributes.onKill
	
	/**
	 * Attributes governing taunt speed and the effects of taunts.
	 */
	override val taunting: TauntingAttributes get() = PDAExpansionDispenserAttributes.taunting
	
	/**
	 * Attributes governing weapon deploy/holster speed, things that activate when a weapon is deployed, and whether a player can swap weapons at all.
	 */
	override val swapWeapons: SwapWeaponsAttributes get() = PDAExpansionDispenserAttributes.swapWeapons
	 ...
	 * For what happens when _this player_ hits an enemy, see [onHit] and [onKill].
	 */
	override val whenHit: WhenHitAttributes get() = PDAExpansionDispenserAttributes.whenHit
	
	override val spyOnly: SpyOnlyAttributes get() = PDAExpansionDispenserAttributes.spyOnly
	
	/**
	 * Attributes related to dealing or preventing critical hits and mini-crits.
	 */
	override val crits: CritsAttributes get() = PDAExpansionDispenserAttributes.crits
	...
	object Inherited : PDAExpansionDispenserAttributes 
}