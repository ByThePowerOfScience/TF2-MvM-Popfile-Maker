...
interface BaseCombatWeaponAttributes : IBlockScoped {
	companion object {
		 ...
		 */
		val useMetalAmmoType = ItemAttributeNamed<Boolean>("mod use metal ammo type")
		 ...
		 */
		val maxPrimaryClipOverride = ItemAttributeNamed<Int>("mod max primary clip override")
		 ...
		 */
		val noReload_displayOnly = ItemAttributeNamed<Float>("mod no reload DISPLAY ONLY")
	 ...
	 * Reminder: non-engies start with 100 metal.
	 */
	val useMetalAmmoType: ItemAttribute<Boolean> get() = BaseCombatWeaponAttributes.useMetalAmmoType
	 ...
	 * Overwrites the max clipsize to a flat value. Applied before other multipliers.
	 */
	val maxPrimaryClipOverride: ItemAttribute<Int> get() = BaseCombatWeaponAttributes.maxPrimaryClipOverride
	 ...
	 * If != 1.0 (if present), says the weapon "does not reload one shot at a time".
	 */
	val noReload_displayOnly: ItemAttribute<Float> get() = BaseCombatWeaponAttributes.noReload_displayOnly
	 ...
	 * Note that for the most part, this logic is set inside the weapon itself. The scattergun thing is weirdly the only way to control this with attributes.
	 */
	val scattergunNoReloadSingle: ItemAttribute<Boolean> get() = BaseCombatWeaponAttributes.scattergunNoReloadSingle
